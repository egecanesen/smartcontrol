...
def record_audio():
    ...
    # Countdown
    print("Say one of these words: 'backward','down','forward','happy','left','right','up'")
    ...
    write(f'resources/sounds/output{time_label}.wav', fs, myrecording)  # Save as WAV file 

    myrecording = torch.from_numpy(myrecording)
    transform = torchaudio.transforms.Resample(orig_freq=fs, new_freq=new_sample_rate)
    transformed = transform(myrecording.T)

    return transformed, time_label
...
def predict(recording):
    tensor = recording.to(device)
    tensor = model(tensor.unsqueeze(0))
    tensor = tensor[:,:,tensor_labels]
    ind = tensor.argmax()
    ind = tensor_labels[ind]
    y_pred = labels[ind]
    return y_pred

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model = M5(n_input=1, n_output=11)
model.load_state_dict(torch.load("resources/unknown_model.pt"))
model.to(device)
model.eval()
labels = ['backward','down','forward','go','happy','left','off','on','right','unknown','up']
tensor_labels = [0, 5, 2, 8, 10, 1, 4]
...
def game_ctrls():
    ...
    while gameControl:
        ...
        msg_screen("Pause: P  (You can also press N if the sound is not predicted correctly)", wheat, 90)
...
def playerfireShell(xy, tankx, tanky, turPost, gun_power, xloc, bar_width, ranHeight, eTankX, eTankY):
    ...
    #msg_screen(f"FIRE {xy}", white, -100, size="small")
    ...
    while fire:
        ...
        if startShell[1] > display_height - grnd_height:
            ...
            msg_screen(f"Impact!", white, -110, size="small")
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_r:
                    ...
                    rec, time_label = record_audio()
                    word = predict(rec)

                    print(f"{word}")
                    #msg_screen("{word}", red, -50, size="medium")
                    ...
                    with open(f'resources/sounds/failure_{time_label}.txt', 'w') as file:
                        pass 


            elif event.type == pygame.KEYUP and event.key == pygame.K_r:
                if word in ["forward", "backward"]:
                    tnkMove = 0
